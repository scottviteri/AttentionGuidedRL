...
\textbf{Loss Function:} The training implements the full chain rule gradient with optional KL regularization to a fixed reference policy:

\begin{align}
\mathcal{L}(\theta) = \mathbb{E}_{\text{batch}} \left[ -\frac{1}{T} \sum_{t=1}^T r_\theta^t - \operatorname{sg}\!\left[\bar{R}(\tau)\right] \sum_{t=1}^T \log \pi_\theta(a_t | s_t) + \beta\, \frac{1}{T} \sum_{t=1}^T D_{\mathrm{KL}}\!\left(\pi_\theta(\cdot\mid s_t) \parallel \pi_{\mathrm{ref}}(\cdot\mid s_t)\right) \right]
\end{align}
...
\begin{itemize}
    ...
    \item The third term is an optional KL regularizer towards a fixed reference policy with coefficient $\beta \ge 0$
\end{itemize}
