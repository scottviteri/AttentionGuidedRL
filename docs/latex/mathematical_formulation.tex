...
\begin{align}
r_\theta^t = \frac{1}{|v_t|} \sum_{i=1}^{|v_t|} \log p_\theta\big(v_{t,i} \mid c_t, k_t, v_{t,<i}\big)
\end{align}
where:
\begin{itemize}
...
\begin{align}
r_\theta^t = \frac{1}{|v_t|} \sum_{i=1}^{|v_t|} \Big( \log p_\theta(v_{t,i} \mid \cdot) - \log p_{\text{ref}}(v_{t,i} \mid \cdot) \Big)
\end{align}
...
\textbf{Loss Function:} The training implements the full chain rule gradient with optional KL regularization on value-token distributions against a fixed reference model:

\begin{align}
\mathcal{L}(\theta) = \mathbb{E}_{\text{batch}} \left[ -\frac{1}{T} \sum_{t=1}^T r_\theta^t - \operatorname{sg}\!\left[\bar{R}(\tau)\right] \sum_{t=1}^T \log \pi_\theta(a_t | s_t) + \beta\, \frac{1}{T} \sum_{t=1}^T D_{\mathrm{KL}}\!\left(p_\theta( v_t\mid c_t, k_t ) \parallel p_{\mathrm{ref}}( v_t\mid c_t, k_t )\right) \right]
\end{align}

where:
\begin{itemize}
    \item $\bar{R}(\tau) = \frac{1}{T} \sum_{t=1}^T r_\theta^t$ is the trajectory average reward
    ...
    \item The third term is an optional KL regularizer on value-token distributions with coefficient $\beta \ge 0$
\end{itemize}
